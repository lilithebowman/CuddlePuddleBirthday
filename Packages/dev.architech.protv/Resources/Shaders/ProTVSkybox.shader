...
Shader "ProTV/Skybox"
{
    Properties
    {
        ...
        [NoScaleOffset] _VideoTex("Video Texture (Render Texture from the TV goes here)", 2D) = "black" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            inline float2 ToCubeCoords(float3 coords, float3 layout, float4 edgeSize, float4 faceXCoordLayouts,
                                       float4 faceYCoordLayouts, float4 faceZCoordLayouts)
            {
                ...
                tcAndLen.xy *= (positiveAndVCross[0]
                                    ? absdir.yx
                                    : (positiveAndVCross[1] ? float2(absdir[2], 0) : float2(0, absdir[2]))) - 0.5;
                ...
                float4 coordLayout = mul(float4(absdir, 0),
                                         float4x4(faceXCoordLayouts, faceYCoordLayouts, faceZCoordLayouts,
                                                  faceZCoordLayouts));
            ...
            v2f vert(appdata_t v)
            {
                ...
                if (_Mapping == 0)
                {
                    ...
                    bool3 aspectTest =
                        sourceAspect >
                        float3(1.0, 1.0f / 6.0f + (3.0f / 4.0f - 1.0f / 6.0f) / 2.0f,
                    ...
                    o.edgeSizeOrLayout3DScaleAndOffset.xy = _MainTex_TexelSize.xy * 0.5 / o.
                        layoutOrImage180ScaleAndCutoff.yz - 0.5;
                ...
                else
                {
                    ...
                    if (_Layout == 0) // No 3D layout
                        ...
                        o.edgeSizeOrLayout3DScaleAndOffset = float4(
                            _SwapEyes ? unity_StereoEyeIndex : 1 - unity_StereoEyeIndex, 0, 0.5, 1);
                    else // Over-Under 3D layout
                        o.edgeSizeOrLayout3DScaleAndOffset = float4(
                            0, _SwapEyes ? unity_StereoEyeIndex : 1 - unity_StereoEyeIndex, 1, 0.5);
                }
                return o;
            }
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                const float2 videoDims = float2(video_width, video_height);
                const bool noVideo = video_width <= 16;

                float2 uv;
                if (_Mapping == 0)
                {
                    uv = ToCubeCoords(i.texcoord, i.layoutOrImage180ScaleAndCutoff, i.edgeSizeOrLayout3DScaleAndOffset,
                                      i.faceXCoordLayouts, i.faceYCoordLayouts, i.faceZCoordLayouts);
                }
                else
                {
                    uv = ToRadialCoords(i.texcoord);
                    if (uv.x > i.layoutOrImage180ScaleAndCutoff[1])
                        return half4(0, 0, 0, 1);
                    uv.x = fmod(uv.x * i.layoutOrImage180ScaleAndCutoff[0], 1);
                    uv = (uv + i.edgeSizeOrLayout3DScaleAndOffset.xy) * i.edgeSizeOrLayout3DScaleAndOffset.zw;
                }

                if (noVideo) uv = uv * _MainTex_ST.xy + _MainTex_ST.zw;

                // sample the texture
                float4 tex;
                if (noVideo) tex = tex2Dlod(_MainTex, float4(uv, 0, 0));
                else tex = _VideoTex.SampleLevel(sampler_VideoTex, uv, 0);

                return tex * _Exposure;
            }
            ENDCG
        }
    }
